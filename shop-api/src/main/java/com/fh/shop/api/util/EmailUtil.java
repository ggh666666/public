...
public class EmailUtil {
    ...
    public static void main(String[] arg) {
        try {
            EmailUtil.sendEmail("532028476@qq.com");//1664046428@qq.com
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail) throws Exception {
        ...
        message.setFrom(new InternetAddress(sendMail, "郭广辉", "UTF-8"));

        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "哈哈", "UTF-8"));

        // 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
        message.setSubject("测试", "UTF-8");

        // 5. Content: 邮件正文（可以使用html标签）（内容有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改发送内容）
        message.setContent("<h1>你的用户已被锁定</h1>", "text/html;charset=UTF-8");
    ...
    public static String myEmailAccount = "2322378199@qq.com";
    public static String myEmailPassword = "lhobkkrnzprediai";//授权码
